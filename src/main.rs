...
enum SgitCommand {
    ...
    /// Reset files (interactive if no flags provided)
    Reset {
        /// Reset all files (staged, unstaged, tracked, untracked)
        #[arg(long)]
        all: bool,
        /// Reset staged files only
        #[arg(long)]
        staged: bool,
        /// Reset unstaged changes only
        #[arg(long)]
        unstaged: bool,
        /// Reset tracked files only
        #[arg(long)]
        tracked: bool,
        /// Reset untracked files only
        #[arg(long)]
        untracked: bool,
    },
...
fn get_untracked_files() -> Result<Vec<String>> {
    let entries = get_porcelain_lines()?;
    let files: Vec<String> = entries
        .into_iter()
        .filter(|(status, _)| {
