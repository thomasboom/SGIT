...
- `sgit push [remote] [branch]` — push against `origin`/current branch by default
- `sgit pull [remote] [branch]` — pull with optional remote/branch

When using `--push`, SGIT will default to `origin` and your current branch name. `--all` stages tracked and untracked files before committing, `--unstaged` stages tracked-but-uncommitted changes, and the plain commit command commits only what you already staged.
